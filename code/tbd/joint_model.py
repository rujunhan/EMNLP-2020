...
class NNClassifier(nn.Module):
    ...
    def predict(self, model, data, args, test=False, gold=True, model_r=None):
        ...
        for doc_id, context_id, sents, ent_keys, ents, poss, rels, lengths in data:
            ...
            if rel_idxs: # predicted relation could be empty --> skip
                ...
                for d, pr, p in zip(doc, pair, predicted.numpy()):
                    key = (d, pr[0], pr[1])
                    if key in args.pair_lookup:
                        event_pair_types = args.pair_lookup[key]
                        pair_counter[event_pair_types] += 1
                        triplet_counter[(event_pair_types[0], event_pair_types[1], self._id_to_label[p])] +=1
        ...
        if test:
            return probs.data, np.mean(losses_t), labels, docs, pairs, f1_score, nopred_rels, event_heads
        else:
    ...
    def _train(self, train_data, eval_data, pos_emb, args):
        ...
        if args.load_model == True:
            checkpoint = torch.load(args.output_dir + args.entity_model_file)
        ...
        for epoch in range(args.epochs):
            ...
            gold = True if epoch < args.pipe_epoch else False
...
class EventEvaluator:
    ...
    def evaluate(self, test_data, args):
        ...
        all_pairs = [(doc + "_" + x, doc + "_" + y) for doc, (x, y) in zip(docs, pairs)]

        sample_event_types = assign_event_types(docs, pairs, args.pair_lookup)
        pred_labels, num_meet_tolerance = LROptimization(preds.numpy(), sample_event_types, true_labels, all_pairs,
                                                         event_heads, self.model._label_to_id, args.constraints,
                                                         args.lr, args.tolerance, args.decay, args.max_iter)
        ...
        if not args.eval_gold:
            ...
            sample_event_types.extend([() for _ in nopred_rels])
            all_pairs.extend(() for _ in nopred_rels)
        assert len(sample_event_types) == len(pred_labels)
        assert len(all_pairs) == len(pred_labels)
...
def get_ratio(triplet_counter, pair_counter):
    triplet_ratio = {}
    for e1, e2, rel in triplet_counter:
        ratio = triplet_counter[(e1, e2, rel)] / pair_counter[(e1, e2)]
        triplet_ratio[(e1, e2, rel)] = ratio
    return triplet_ratio
...
def compute_ratio(split, filedir):
    ...
    triplet_ratio = get_ratio(triplet_counter, pair_counter)
    return triplet_ratio, pair_lookup

def parse_constraint_file(input_file):

    constraints = [line.strip().split(',') for line in open(input_file)]
    return [(x[0], x[1], x[2]) for x in constraints if x[-1] == 'Y']
...
def main(args):
    ...
    test_data = EventDataset(args.data_dir + type_dir, args.test_split)
...
if __name__ == '__main__':
    ...
    p.add_argument('-pipe_epoch', type=int, default=-1)
    p.add_argument('-seed', type=int, default=123)
    p.add_argument('-lr', type=float, default=0.0005)
    p.add_argument('-num_classes', type=int, default=2) # updated in main()
    ...
    p.add_argument('-entity_model_file', type=str, default="pipeline_best.pth.tar")
    ...
    ############################################
    # Hyper-paramters used in LR optimization #
    ############################################
