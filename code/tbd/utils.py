...
def assign_event_types(docs, pairs, event_types):
    res = []
    for doc, pair in zip(docs, pairs):
        pair_key = (doc, pair[0], pair[1])
        if pair_key in event_types:
            res.append(event_types[pair_key])
        else:
            res.append(())
    return res
